  ...
  return (
    <section className="py-16 md:py-24 bg-[#1E1E1E]">
      <div className="max-w-7xl mx-auto px-4 md:px-6">
        ...
        >
          <h2 className="text-[#B2FF59] mb-3 md:mb-4 text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-extrabold font-tech uppercase tracking-wider">Forecast Explorer</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          ...
          >
            ...
            <div className="space-y-2 md:space-y-3">
              {(Object.keys(CHART_CONFIGS) as Array<keyof typeof CHART_CONFIGS>).map((chartType) => (
                <button
                  ...
                  className={`w-full text-left p-3 md:p-4 rounded-lg md:rounded-xl transition-all duration-300 min-h-[44px] text-sm md:text-base ${
            ...
            <motion.div
              ...
              className="bg-[#2B2B2B] p-4 md:p-6 rounded-lg md:rounded-xl border border-[#B2FF59]/20"
            >
              <h4 className="text-[#B2FF59] mb-3 md:mb-4 text-base md:text-lg font-bold">2030 Projection</h4>
              <div className="space-y-2 md:space-y-3">
                <div>
                  <div className="text-[#FAFAFA]/60 text-xs md:text-sm">Target Value</div>
                  <div className="text-[#FAFAFA] font-mono mt-1 text-base md:text-lg">
                    {CAPACITY_DATA[CAPACITY_DATA.length - 1][config.dataKey]} {config.unit}
                  </div>
                </div>
                <div>
                  <div className="text-[#FAFAFA]/60 text-xs md:text-sm">Growth Rate</div>
                  <div className="text-[#B2FF59] font-mono mt-1 text-base md:text-lg">
                    +
                    {(
                ...
                <div>
                  <div className="text-[#FAFAFA]/60 text-xs md:text-sm">Confidence Level</div>
                  <div className="text-[#1565C0] font-mono mt-1 text-base md:text-lg">High</div>
                </div>
              </div>
            </motion.div>

            {/* Data Source Badge */}
            <div className="bg-[#2B2B2B] p-3 md:p-4 rounded-lg md:rounded-xl border border-[#1565C0]/20">
              <div className="flex items-center gap-2 mb-2">
                <span className="px-2 py-1 bg-[#B2FF59]/20 text-[#B2FF59] text-xs rounded">VERIFIED</span>
                <span className="text-[#FAFAFA]/60 text-xs md:text-sm">Data Sources</span>
              </div>
              <p className="text-[#FAFAFA]/70 text-xs md:text-sm">
          ...
          <motion.div
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.6 }}
            className="lg:col-span-3 bg-[#2B2B2B] p-4 sm:p-6 md:p-8 rounded-lg md:rounded-xl border border-[#B2FF59]/20"
          >
            <motion.div
              key={activeChart}
              ...
              transition={{ duration: 0.4 }}
            >
              <h3 className="text-[#FAFAFA] mb-4 md:mb-6 text-lg md:text-xl font-bold">{activeChart}</h3>
              <ResponsiveContainer width="100%" height={300} className="sm:hidden">
                <AreaChart data={CAPACITY_DATA}>
                  ...
                  <XAxis
                    dataKey="year"
                    stroke="#FAFAFA"
                    style={{ fontSize: '10px', fill: '#FAFAFA' }}
                  />
                  <YAxis
                    stroke="#FAFAFA"
                    style={{ fontSize: '10px', fill: '#FAFAFA' }}
            ...
            <div className="mt-4 md:mt-6 flex justify-end">
              <button className="w-full sm:w-auto px-4 md:px-6 py-2.5 md:py-3 bg-[#B2FF59] text-[#1E1E1E] rounded-lg hover:bg-[#A0E050] transition-colors duration-300 text-sm md:text-base font-semibold min-h-[44px]">
