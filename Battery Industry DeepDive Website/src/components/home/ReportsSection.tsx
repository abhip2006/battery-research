  ...
  return (
    <section className="py-24 bg-[#2B2B2B]">
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          <h2 className="text-[#B2FF59] mb-4">Policy & Research Reports</h2>
          ...
          {REPORTS.map((report, index) => (
            ...
            >
              <div
                ...
                className="bg-[#1E1E1E] p-6 rounded-xl border border-[#B2FF59]/20 hover:border-[#B2FF59] cursor-pointer transition-all duration-300 hover:shadow-lg hover:shadow-[#B2FF59]/10"
              >
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-start gap-4 mb-3">
                      <FileText className="size-6 text-[#B2FF59] flex-shrink-0 mt-1" />
                      <div className="flex-1">
                  ...
                  <button className="text-[#B2FF59] text-sm opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    View Summary →
                  </button>
                </div>
              </div>
        ...
        {selectedReport && (
          <>
            ...
            <motion.div
              ...
              className="fixed right-0 top-0 bottom-0 w-[600px] bg-[#1E1E1E] z-50 shadow-2xl overflow-y-auto"
            >
              <div className="p-8">
                {/* Header */}
                <div className="flex items-start justify-between mb-6">
                  ...
                  <button
                    onClick={() => setSelectedReport(null)}
                    className="p-2 hover:bg-[#2B2B2B] rounded-lg transition-colors duration-300"
                  >
                    <X className="size-6 text-[#FAFAFA]" />
                  </button>
                </div>
                ...
                {REPORTS.filter((r) => r.id === selectedReport).map((report) => (
                  <div key={report.id} className="space-y-6">
                    <div>
                      <h2 className="text-[#FAFAFA] mb-4">{report.title}</h2>
                      <div className="flex items-center gap-4 text-sm text-[#FAFAFA]/60 mb-4">
                        <span className="flex items-center gap-1">
                          <ExternalLink className="size-3" />
                          {report.source}
                        </span>
                      ...
                      <div className="flex gap-2">
                        {report.tags.map((tag) => (
                          <span
                            key={tag}
                            className="px-3 py-1 bg-[#B2FF59]/20 text-[#B2FF59] text-sm rounded"
                          >
                            {tag}
                          </span>
                        ))}
                    ...
                    <div>
                      <h3 className="text-[#B2FF59] mb-3">Key Insights</h3>
                      <p className="text-[#FAFAFA]/80 leading-relaxed">{report.summary}</p>
                    </div>

                    <div className="bg-[#2B2B2B] p-6 rounded-xl border border-[#B2FF59]/20">
                      <h4 className="text-[#B2FF59] mb-3">Report Statistics</h4>
                    ...
                    <button className="w-full px-6 py-3 bg-[#B2FF59] text-[#1E1E1E] rounded-lg hover:bg-[#A0E050] transition-colors duration-300">
